#!/usr/bin/env python3
"""Run the retrieval evaluation and write a baseline.

Offline. Builds a throwaway database, ingests the synthetic evaluation corpus,
runs every fixture through the same ``retrieve()`` the planner calls, and writes
metrics with Wilson intervals to eval/baselines/retrieval.json.

    python3 backend/scripts/run_retrieval_eval.py
    python3 backend/scripts/run_retrieval_eval.py --write

Without --write it prints the report and changes nothing, which is what CI runs.
"""
...
from suas.db.models import Base, CorpusChunkRow
...
async def run() -> dict[str, Any]:
    """Ingest the eval corpus, run every fixture, and return the report."""
    ...
    async with session_factory() as session:
        ...
        # wrong_config_leak vacuously zero -- it would be measuring the query,
        # not the filter.
